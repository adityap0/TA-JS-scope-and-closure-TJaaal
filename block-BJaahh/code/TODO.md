...
```js
function addGame(gameName, score = 0) {
  function finalFn() {
    score = score + 1;
...
function getCard(suit) {
  ...
  function finalFn() {
    return `Card is : ${suit} ${arr[Math.floor(Math.random() * arr.length)]}`;
  }
  return finalFn;
}
